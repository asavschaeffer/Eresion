...
from shared.interfaces import Token, TokenType, ITokenizer
from text_based_rpg.game_logic.state import GameState, WorldStateSnapshot
...
class StreamlinedTokenizer(ITokenizer):
    ...
    def process_world_state(self, snapshot: WorldStateSnapshot) -> List[Token]:
        """
        Generate tokens from world state snapshot.
        
        This creates contextual tokens to supplement the rich action tokens
        generated by individual D&D actions.
        """
        tokens = []
        ...
        if game_state.social.recent_conversations:
        ...
        # 6. Process discrete events (mainly for action tokens)
        ...
        if self.config.debug_tokenization:
            print(f"[STREAMLINED_TOKENIZER] Generated {len(tokens)} context tokens")
        
        return tokens
    
    def _create_location_token(self, game_state: GameState, timestamp: float) -> Token:
        ...
        return Token(
            ...
            metadata={
                ...
                "location_type": self._classify_location(game_state.player.location),
                "safety_level": self._assess_location_safety(game_state),
                "entities_present": len(game_state.environment.current_location_entities)
            }
        )
    
    def _create_resource_token(self, game_state: GameState, timestamp: float) -> Token:
        """Create resource state token."""
        health_category = self._categorize_resource(game_state.player.health_percent)
        stamina_category = self._categorize_resource(game_state.player.stamina_percent)
        ...
        return Token(
            ...
            metadata={
                ...
                "buff_count": len(game_state.player.active_buffs)
            }
        )
    
    def _create_combat_token(self, game_state: GameState, timestamp: float) -> Token:
        """Create combat context token."""
        hostile_entities = [e for e in game_state.environment.current_location_entities.values() if e.is_hostile]
        ...
        return Token(
            ...
            metadata={
                ...
                "player_health": game_state.player.health_percent,
                "player_stamina": game_state.player.stamina_percent,
                "threat_level": self._assess_threat_level(hostile_entities, game_state),
                "combat_duration": getattr(game_state, 'combat_duration', 0)
            }
        )
    
    def _create_social_token(self, game_state: GameState, timestamp: float) -> Token:
        """Create social context token."""
        recent_conv = game_state.social.recent_conversations[-1] if game_state.social.recent_conversations else {}
        ...
        return Token(
            ...
            metadata={
                ...
                "total_relationships": len(game_state.social.relationship_scores),
                "average_relationship": self._calculate_average_relationship(game_state),
                "social_activity_level": len(game_state.social.recent_conversations)
            }
        )
    
    def _create_environmental_token(self, game_state: GameState, timestamp: float) -> Token:
        ...
        return Token(
            ...
            metadata={
                ...
                "session_number": getattr(game_state, 'current_session', 1),
    ...
    def _assess_location_safety(self, game_state: GameState) -> float:
    ...
    def _calculate_resource_pressure(self, game_state: GameState) -> float:
    ...
    def _assess_threat_level(self, hostile_entities: List, game_state: GameState) -> float:
    ...
    def _calculate_average_relationship(self, game_state: GameState) -> float:
    ...
    def _calculate_environmental_pressure(self, game_state: GameState) -> float:
